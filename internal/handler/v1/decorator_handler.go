...
// DecoratorHandler 实现 decorator.v0.Decorator：把榜单 member（此处为文章 ID 字符串）
// 装饰为展示信息（title / slug / cover_image / user_id / author_name），供 ranking-service
// 在 GetRanking 时回调。
//
// 关键设计：展示字段的语义由本服务（数据拥有方）定义，ranking-service 仅透传
// google.protobuf.Struct，不感知 title/author_name 等字段，因此新增榜单无需改动它。
...
func (h *DecoratorHandler) Decorate(ctx context.Context, req *decoratorv0pb.DecorateRequest) (*decoratorv0pb.DecorateResponse, error) {
	...
	arts, err := h.Repo.GetArticlesByIDs(ctx, ids)
	if err != nil {
		log.Warnf("[decorator] get articles by ids failed: %v", err)
		return &decoratorv0pb.DecorateResponse{}, nil
	}

	display := make(map[string]*structpb.Struct, len(arts))
	for _, a := range arts {
		...
		display[strconv.FormatUint(uint64(a.ID), 10)] = s
	}
